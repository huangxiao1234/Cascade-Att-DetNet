...
def vis_detections(f,img_name,im, class_name, dets, thresh=0.8):
    
    """Visual debugging of detections."""
    for i in range(np.minimum(10, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = round(dets[i, -1],6)
        if score > thresh:
            print(img_name,bbox[0], bbox[1],bbox[2],bbox[3],score)
            f.append([img_name,bbox[0], bbox[1],bbox[2],bbox[3],score])
...
def parse_args():
    ...
    parser.add_argument('--csvname', dest='csvname',
                        help='detnet59, etc',
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(load_name)
    fpn.load_state_dict({k: v for k, v in checkpoint['model'].items() if k in fpn.state_dict()})
    ...
    for i in range(num_images):
        ...
        for j in xrange(1, len(classes)):
            ...
            if vis:
                im2show = vis_detections(final_result,imglist[i],im2show, classes[j], cls_dets, thresh=args.thresh)
    ...
    d.columns=['filename','xmin','ymin','xmax','ymax','score']
    d = d.round({'score':6})
    d.to_csv(args.csvname,index=False)
