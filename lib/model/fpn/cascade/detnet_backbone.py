...
class DetNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.attention1 = self._make_attention(64, 256,(150,200))
        self.attention2 = self._make_attention(256, 512,(75,100))
        self.attention3 = self._make_attention(512, 1024,(38,50))
        self.attention4 = self._make_attention(1024, 1024,(38,50))
        self.attention5 = self._make_attention(1024, 1024,(38,50))
    ...
    def _make_attention(self,inc,outc,in_size):
        
        
        layers = []
        layers.append(nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        layers.append(nn.UpsamplingBilinear2d(size=in_size))
        layers.append(nn.Conv2d(inc,inc,kernel_size=1))
        layers.append(nn.Conv2d(inc,outc,kernel_size=1))
    ...
    def forward(self, x):
        ...
        x1 = self.layer1(x)
        a1 = self.attention1(x)
        x = x1.mul(a1)+x
        x2 = self.layer2(x)
        a2 = self.attention2(x)
        x = x2.mul(a2)+x
        x3 = self.layer3(x)
        a3 = self.attention3(x)
        x = x3.mul(a3)+x
        x4 = self.layer4(x)
        a4 = self.attention4(x)
        x = x4.mul(a4)+x
        x5 = self.layer5(x)
        a5 = self.attention5(x)
        x = x5.mul(a5)+x
...
class detnet(_FPN):
    ...
    def _init_modules(self):
        ...
        if self.pretrained == True:
            ...
            model_dict = detnet.state_dict()
          
            model_dict.update({k: v for k, v in state_dict.items() if k in model_dict}) 
            detnet.load_state_dict(model_dict)

            # detnet.load_state_dict({k: v for k, v in state_dict.items() if (k in detnet.state_dict()) and ('attention' not in k)})
        ...
        self.RCNN_attention1 = nn.Sequential(detnet.attention1)
        self.RCNN_attention2 = nn.Sequential(detnet.attention2)
        self.RCNN_attention3 = nn.Sequential(detnet.attention3)
        self.RCNN_attention4 = nn.Sequential(detnet.attention4)
        self.RCNN_attention5 = nn.Sequential(detnet.attention5)        
    ...
    def train(self, mode=True):
        ...
        if mode:
            ...
            self.RCNN_attention1.train()
            self.RCNN_attention2.train()
            self.RCNN_attention3.train()
            self.RCNN_attention4.train()
            self.RCNN_attention5.train()
            ...
            self.RCNN_attention1.apply(set_bn_eval)
            self.RCNN_attention2.apply(set_bn_eval)
            self.RCNN_attention3.apply(set_bn_eval)
            self.RCNN_attention4.apply(set_bn_eval)
            self.RCNN_attention5.apply(set_bn_eval)
